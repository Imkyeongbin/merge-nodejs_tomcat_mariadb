...
export ODR_HTTP_PORT_NUMBER="$AGENT_PORT"
# export ODR_PORT_A="$PORT_A"
#sed -i "s/ODR_HTTP_PORT_NUMBER:-18019/ODR_HTTP_PORT_NUMBER:-$PORT_A/g" /opt/bitnami/scripts/libtomcat.sh
...
#env 파트
cp /nodeApp/.env /tmp
sed -i "s/18036/$DB_PORT/g" /tmp/.env
\cp -f /tmp/.env /nodeApp/
#sed -i "s/23306/$PORT_C/g" /opt/bitnami/scripts/mariadb-env.sh

#cp /userContent/delay.sh /tmp
sed -i "s/timeout 180/timeout $DELAY_TIME/g" /userContent/delay.sh
#\cp -f /tmp/delay.sh /userContent

#ocr address 변경
export OCR_ADDRESS_AGENT=${OCR_ADDRESS_AGENT_CHANGED:-$OCR_ADDRESS_AGENT}

#ocr direct address 변경
export OCR_ADDRESS_DIRECT=${OCR_ADDRESS_DIRECT_CHANGED:-$OCR_ADDRESS_DIRECT}/yourAddress
